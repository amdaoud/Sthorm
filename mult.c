...
int *jobs;/*pointeur contenant le nbre de taches effectuées par chaque thread*/ 
...
{
void *status ;
/*récupérer le nbre de threads*/

pthread_t* threadid;
pthread_attr_t attr;
pthread_attr_setscope (&attr, PTHREAD_SCOPE_SYSTEM) ;
threadid = (pthread_t *) malloc (sizeof(pthread_t) * nbr_thread) ;
jobs = (int *) malloc (sizeof(int) * nbr_thread) ;
...
for( i = 0; i < nbr_thread; i++)
   ...
   pthread_create(&*(threadid+1),NULL,(void *(*) (void *))work,(void *)(intptr_t) i);
...
if (*(jobs+i)>max)
 max=*(jobs+i);*/
 //printf ("Worker [%2d] terminates, %4d jobs.\n",i,*(jobs+i)) ;
}
